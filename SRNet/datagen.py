...
class datagen_srnet(Dataset):
    def __init__(self, cfg, torp = 'train', transforms = None):
        ...
        if(torp == 'train'):
            ...
            self.folder_list = os.listdir(os.path.join(self.data_dir, self.t_b_dir))
            self.name_list = []
            for folder in self.folder_list:
                self.name_list += os.listdir(os.path.join(self.data_dir, self.t_b_dir, folder))
    ...
    def __getitem__(self, idx):
        ...
        i_t = io.imread(os.path.join(cfg.data_dir, cfg.i_t_dir, str(idx//1000), img_name))
        i_s = io.imread(os.path.join(cfg.data_dir, cfg.i_s_dir, str(idx//1000), img_name))
        t_sk = io.imread(os.path.join(cfg.data_dir, cfg.t_sk_dir, str(idx//1000), img_name), as_gray = True)
        t_t = io.imread(os.path.join(cfg.data_dir, cfg.t_t_dir, str(idx//1000), img_name))
        t_b = io.imread(os.path.join(cfg.data_dir, cfg.t_b_dir, str(idx//1000), img_name))
        t_f = io.imread(os.path.join(cfg.data_dir, cfg.t_f_dir, str(idx//1000), img_name))
        mask_t = io.imread(os.path.join(cfg.data_dir, cfg.mask_t_dir, str(idx//1000), img_name), as_gray = True)
...
class example_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        i_t = np.asarray(Image.open(os.path.join(cfg.example_data_dir, img_name + 'i_t.png')).convert('RGB'))

        #i_s = io.imread(os.path.join(cfg.example_data_dir, img_name + 'i_s.png'))
        i_s = np.asarray(Image.open(os.path.join(cfg.example_data_dir, img_name + 'i_s.png')).convert('RGB'))
        h, w = i_t.shape[:2]
