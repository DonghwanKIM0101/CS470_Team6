...
def main():
    ...
    try:
      ...
      print('load from', cfg.ckpt_path)
      cur_step = cfg.ckpt_path[cfg.ckpt_path.index('-')+1:cfg.ckpt_path.index('.')]
    ...
    for step in tqdm(range(cfg.max_iter)):
        ...
        if ((step+1) % cfg.save_ckpt_interval == 0):
            ...
            torch.save(
                ...
                cfg.checkpoint_savedir+f'train_step-{int(cur_step)+step+1}.model',
            )
            print('save to', cfg.checkpoint_savedir, 'train_step-%d.model'%(int(cur_step)+step+1))
