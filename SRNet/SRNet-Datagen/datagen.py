...
def main():
    ...
    for idx in range(cfg.sample_num):
        if idx%1000 == 0:
            i_t_dir_temp = os.path.join(i_t_dir, str(idx//1000))
            i_s_dir_temp = os.path.join(i_s_dir, str(idx//1000))
            t_sk_dir_temp = os.path.join(t_sk_dir, str(idx//1000))
            t_t_dir_temp = os.path.join(t_t_dir, str(idx//1000))
            t_b_dir_temp = os.path.join(t_b_dir, str(idx//1000))
            t_f_dir_temp = os.path.join(t_f_dir, str(idx//1000))
            mask_t_dir_temp = os.path.join(mask_t_dir, str(idx//1000))
        
        makedirs(i_t_dir_temp)
        makedirs(i_s_dir_temp)
        makedirs(t_sk_dir_temp)
        makedirs(t_t_dir_temp)
        makedirs(t_b_dir_temp)
        makedirs(t_f_dir_temp)
        makedirs(mask_t_dir_temp)    
            
        print ("Generating step {:>6d} / {:>6d}".format(idx + 1, cfg.sample_num))
        i_t, i_s, t_sk, t_t, t_b, t_f, mask_t = mp_gen.dequeue_data()
        i_t_path = os.path.join(i_t_dir_temp, str(idx).zfill(digit_num) + '.png')
        i_s_path = os.path.join(i_s_dir_temp, str(idx).zfill(digit_num) + '.png')
        t_sk_path = os.path.join(t_sk_dir_temp, str(idx).zfill(digit_num) + '.png')
        t_t_path = os.path.join(t_t_dir_temp, str(idx).zfill(digit_num) + '.png')
        t_b_path = os.path.join(t_b_dir_temp, str(idx).zfill(digit_num) + '.png')
        t_f_path = os.path.join(t_f_dir_temp, str(idx).zfill(digit_num) + '.png')
        mask_t_path = os.path.join(mask_t_dir_temp, str(idx).zfill(digit_num) + '.png')
